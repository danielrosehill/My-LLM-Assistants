...
def initialize_cache_from_readme():
    ...
    try:
        ...
        for row in rows:
            ...
            if len(parts) != 5:  # Now 5 columns with creation date
                continue
                
            title, creation_date, description, repo_link, hf_link = [p.strip() for p in parts]
...
def get_markdown_files():
    ...
    for filepath in BASE_PATH.rglob('*.md'):
        files.append((filepath, get_creation_date(filepath)))
    
    # Sort by creation date, newest first
    files.sort(key=lambda x: datetime.strptime(x[1], '%B %d, %Y'), reverse=True)
    return [(str(f.relative_to(REPO_ROOT)), d) for f, d in files]
...
def generate_table(files):
    print("\nGenerating table...")
    table = "| Model | Created | Description | Repository | HuggingFace |\n"
    table += "|--------|----------|-------------|------------|-------------|\n"
    ...
    for index, (rel_file, file_date) in enumerate(files, 1):
        ...
        try:
            ...
            if rel_file in cache and cache[rel_file]['hash'] == file_hash:
                ...
                table += f"| {cached_data['title']} | {cached_data['creation_date']} | {cached_data['description']} | {cached_data['repo_link']} | {cached_data['hf_link']} |\n"
                cached_count += 1
                continue
            ...
            table += f"| {title} | {file_date} | {description} | {repo_link} | {hf_link} |\n"
            processed_count += 1
